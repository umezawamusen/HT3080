...
#define FLOW_NO_CTRL 0
#define FLOW_CTRL_HARD 1
#define FLOW_CTRL_SOFT 2
...
void ht3080_set_rs485_mode(int port, int set);
...
int ht3080_open(void)
{
	...
	for ( i = 0; i < MAX_TTY; ++i )
	{
		...
		fd_ht3080[i] = open(tty, O_RDWR | O_NOCTTY | O_NONBLOCK);
		if ( fd_ht3080[i] < 0 )
		{
